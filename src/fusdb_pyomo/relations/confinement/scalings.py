...
def tau_E_hubbard_nominal(I_p: float, B0: float, n_la: float, P_heating: float) -> Any:
    """
...
def tau_E_murari(I_p: float, R: float, kappa_ipb: float, n_la: float, B0: float, P_loss: float) -> float:
    ...
    return (
        ...
        * kappa_ipb ** 1.450
        * P_loss_MW ** (-0.735)
        * (dnla19 ** 0.448 / (1.0 + np.exp(-9.403 * (dnla19 / B0) ** -1.365)))
    )
...
def iter_h90p_amended_confinement_time(I_p: float, B0: float, afuel: float, R: float, P_loss: float, kappa: float) -> float:
    ...
    I_p_MA = I_p / 1e6
    P_loss_MW = P_loss / 1e6
    return 0.082 * I_p_MA ** 1.02 * B0 ** 0.15 * np.sqrt(afuel) * R ** 1.60 / (P_loss_MW ** 0.47 * kappa ** 0.19)
...
def jaeri_confinement_time(
    ...
    return (
        0.085 * kappa_95 * a**2 * np.sqrt(afuel)
        + 0.069
        * n20 ** 0.6
        * I_p_MA
        * B0 ** 0.2
...
def goldston_confinement_time(I_p: float, R: float, a: float, kappa_95: float, afuel: float, P_loss: float) -> float:
    ...
    I_p_MA = I_p / 1e6
    P_loss_MW = P_loss / 1e6
    return 0.037 * I_p_MA * R ** 1.75 * a ** (-0.37) * np.sqrt(kappa_95) * np.sqrt(afuel / 1.5) / np.sqrt(P_loss_MW)
...
def rebut_lallia_confinement_time(
    ...
    term1 = 1.2e-2 * I_p_MA * rll ** 1.5 / np.sqrt(Z_eff)
    term2 = 0.146 * dnla20 ** 0.75 * np.sqrt(I_p_MA) * np.sqrt(B0) * rll ** 2.75 * Z_eff ** 0.25 / P_loss_MW
    return 1.65 * np.sqrt(afuel / 2.0) * (term1 + term2)
...
def tau_E_mirnov(a: float, kappa_95: float, I_p: float) -> float:
    ...
    """
    I_p_MA = I_p / 1e6
    return 0.2 * a * np.sqrt(kappa_95) * I_p_MA
