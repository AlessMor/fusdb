...
def calc_synchrotron_radiation(
    ...
    The supplied profile values are integrated over physical volume through
    ``w_V`` when available. ``rho`` remains the computational sampling grid;
    omitting ``w_V`` preserves the historical self-similar volume weighting.
    """
    # CHECK
    ne20 = n_e / 1e20
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        ...
        p_sync = 6.25e-3 * ne20 * T_e * B0**2 * Phi * 1e6
