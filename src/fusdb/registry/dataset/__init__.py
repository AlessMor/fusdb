...
def _index_datasets(resource_root: Path | None = None) -> Mapping[str, Path]:
    """Index packaged YAML datasets by stable filename stem."""
    root = Path(resource_root) if resource_root is not None else Path(str(files(__package__)))
    index: dict[str, Path] = {}
    for path in sorted(root.rglob("*.yaml")):
        dataset_id = path.stem
        if dataset_id in index:
            raise ValueError(f"Duplicate dataset id {dataset_id!r}: {index[dataset_id]} and {path}.")
        index[dataset_id] = path
    return MappingProxyType(index)


DATASETS = _index_datasets()


def resolve_dataset(dataset_ref: str | Path) -> Path:
    """Resolve a stable dataset ID, canonical filename, or explicit path."""
...
def _validate_document(path: Path, raw: Any, *, expected_datatype: str | None) -> DatasetDocument:
    ...
    missing = [key for key in ("schema_version", "datatype", "source", "subject") if key not in raw]
    if missing:
    ...
    if version != SCHEMA_VERSION:
        raise ValueError(f"Dataset {path.name!r} uses schema_version {version!r}; expected {SCHEMA_VERSION}.")
    ...
    if expected_datatype is not None and datatype != expected_datatype:
        raise ValueError(f"Dataset {path.name!r} uses datatype {datatype!r}; expected {expected_datatype!r}.")
    ...
    if path.name != expected_name:
        raise ValueError(f"Dataset {path.name!r} does not follow its declared canonical name {expected_name!r}.")
...
def load_dataset(dataset_ref: str | Path, *, expected_datatype: str | None = None) -> DatasetDocument:
    """Resolve, parse and validate one scientific dataset."""
    return _load_dataset_cached(resolve_dataset(dataset_ref), expected_datatype)
...
def load_amjuel_h4_fit(dataset_ref: str | Path) -> dict[str, Any]:
    ...
    return {**document.data, "path": document.path, "coefficients": coefficients,
            "parsed_density_limits_cm3": parsed_density_limits}


@lru_cache(maxsize=None)
def load_table(dataset_ref: str | Path, *, metadata_keys: tuple[str, ...] = ()) -> LoadedTable:
    ...
    for index, column_spec in enumerate(document.data["columns"]):
        if not isinstance(column_spec, dict):
            raise ValueError(f"Dataset {document.path.name!r} column {index} must be a mapping.")
        name = column_spec.get("name")
        unit = column_spec.get("unit")
        if not isinstance(name, str) or not isinstance(unit, str):
            raise ValueError(f"Dataset {document.path.name!r} column {index} needs string name and unit.")
    ...
    for line_number, row in enumerate(csv.reader(StringIO(document.data["data"])), start=1):
        ...
        if len(row) != len(quantities):
            raise ValueError(f"Dataset {document.path.name!r} CSV row {line_number} has {len(row)} columns; expected {len(quantities)}.")
    ...
    return LoadedTable(
        document.dataset_id, document.path, metadata, tuple(quantities), tuple(units),
        tuple(np.asarray(values, dtype=float) for values in values_by_column),
    )


__all__ = [
    "DATASETS", "SCHEMA_VERSION", "SUPPORTED_DATATYPES", "DatasetDocument", "LoadedTable",
    "dataset_filename", "load_amjuel_h2_fit", "load_amjuel_h4_fit", "load_dataset",
    "load_table", "parse_fortran_float", "resolve_dataset",
]
