<section class="home-hero" data-home-hero>
  <div class="home-hero__media" aria-hidden="true"></div>
  <div class="home-hero__veil"></div>
  <div class="home-hero__content">
    <!-- <p class="home-hero__eyebrow">Fusion reactor scenario database and solver</p> -->
    <h1>fusdb</h1>
    <p class="home-hero__lead">
      A lightweight and extensible database for nucelar fusion data and relations used for reactor analysis.
    </p>.
  </div>
</section>

<div class="home-grid">
  <a class="home-card" href="getting_started/">
    <span class="home-card__label">🏁 Getting Started</span>
    <span>Install `fusdb`, load and browse reactors, and use the main tools.</span>
  </a>

  <a class="home-card" href="knowledge_base/">
    <span class="home-card__label">📖 Knowledge Base</span>
    <span>Physics notes, definitions, and modelling context.</span>
  </a>

  <a class="home-card" href="developers_guide/">
    <span class="home-card__label">🔧 Developer's Guide</span>
    <span>Contribute and extend FusDB.</span>
  </a>

  <a class="home-card" href="code_docs/">
    <span class="home-card__label">🖥️ Source Documentation</span>
    <span>API reference and code overview.</span>
  </a>
  
  <a class="home-card" href="bibliography/">
    <span class="home-card__label">📚 Bibliography</span>
    <span>List of references used in the documentation and code.</span>
  </a>
  
  <!-- TODO: add reactors and relationsystem cards -->
  <a class="home-card" href="getting_started/reactors.md">
    <span class="home-card__label">💥 Reactors</span>
    <span></span>
    
  <a class="home-card" href="code_docs/relationsystem_class.md">
    <span class="home-card__label">🌐 RelationSystem</span>
    <span>The core of FusDB</span>
  </a>

  </a>
</div>

<!-- 
## Quick Paths

- [Installation and usage guide](getting_started/index.md)
- [Reactor reference](code_docs/reactors/index.md)
- [Cross sections and reactivities](knowledge_base/plasma_physics/0-fusion_plasmas/cross_sections_reactivities.md)
- [Developer's Guide](developers_guide/index.md)
- [Source documentation overview](code_docs/index.md)
- [Bibliography](bibliography/index.md) -->
