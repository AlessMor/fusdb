...
# Densities chosen distinct and non-round so an accidentally symmetric
# coefficient error cannot cancel.
...
def _expected_rate(spec, sigmav: float) -> float:
    """n_a * n_b * <sigma v>, halved for like reactants (double counting)."""
    a, b = spec.reactants
    rate = DENSITIES[a] * DENSITIES[b] * sigmav
    return 0.5 * rate if spec.like_reactants else rate


@pytest.mark.parametrize("key", sorted(REACTIONS.keys()))
def test_balance_ode_matches_registry_stoichiometry(key: str) -> None:
    """Fire one reaction at a time and check every species' net coefficient."""
    spec = REACTIONS[key]
    sigmav = 1.0e-22
    ...
    for species, actual in zip(BALANCE_SPECIES, balances):
        ...
        assert actual == pytest.approx(expected, rel=1e-12, abs=0.0), (
            f"{key}: d{species}/dt coefficient is "
            f"{actual / rate if rate else actual}, reactions.yaml says "
            f"{spec.stoichiometry(species)}"
        )


def test_neutron_rate_matches_registry_multiplicities() -> None:
    """``neutron_production_rate`` weights each channel by its neutron yield."""
    ...
    for spec in REACTIONS:
        ...
        if yield_n <= 0:
            assert argument not in args, (
                f"{spec.key} produces no neutrons but is summed into neutron_rate"
            )
            continue
        assert argument in args, (
            f"{spec.key} yields {yield_n} neutron(s) in reactions.yaml but is "
            "missing from neutron_production_rate"
        )
...
def test_charged_and_neutron_power_partition_every_product() -> None:
    """Every product of every reaction lands in exactly one power aggregate.

    Guards the case the aggregates were written for by hand: adding a reaction
    to reactions.yaml without wiring its products into P_charged / P_neutron.
    """
    ...
    for spec in REACTIONS:
        for product in set(spec.products):
            if spec.energies_j.get(product) is None:
                continue  # product carries no energy term of its own
            target = neutral if product == "n" else charged
            assert any(
                name.startswith(f"P_fus_{spec.key}") for name in target
            ), f"{spec.key} product {product!r} is not aggregated into fusion power"


def test_registry_species_are_known() -> None:
    """Every reaction participant is a registry species.

    ``n`` has its own row and ``p`` is an alias of ``H`` (a fully-ionised
    hydrogen atom is a proton), so no participant needs an escape hatch.
    """
    for spec in REACTIONS:
...
# The atomic-data species tuples used to be hardcoded here and checked only for
# spelling; they are now derived from the registry's ``atomic_data`` and are
# covered by tests/test_species_registry.py.
